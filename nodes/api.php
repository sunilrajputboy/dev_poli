				...
				foreach ($colorArray as $keyNo => $clr) {
					...
					$maxNew  = $max - $keyInterval;
						...
						if($min_key_values != null && $max_key_values != null && count($max_key_values) > 0 && count($min_key_values) > 0){
						       if($keyNo === 0){
								$keyClr['MinDisplayKey'] = (isset($min_display_values[$keyNo]) ? $min_display_values[$keyNo] :$min_key_values[$keyNo]) . ($d['show_last_key'] == 'Yes' ? '': '+');
							
								$keyClr['minKeyValue'] = $min_key_values[$keyNo] . ($d['show_last_key'] == 'Yes' ? '': '+');
