...
// 		$query = "SELECT map_template_regions.name, map_template_regions.fname, map_template_regions.lname, map_template_regions.party, map_template_regions.twitter_handle, map_template_regions.prefix, map_template_regions.email, map_template_regions.profile_url, project_fields.field_name, data_field_value.field_value FROM `data_field_value`,project_fields,map_template_regions WHERE  data_field_value.`pro_id`='".$project_id."' AND map_template_regions.id=data_field_value.city_id AND data_field_value.field_id=project_fields.id_project_field AND project_fields.isGroup=0 AND data_field_value.city_id IN(SELECT DISTINCT(`city_id`) FROM `data_field_value` WHERE `pro_id`='$project_id') ORDER BY `data_field_value`.`city_id` ASC";
		$query = "SELECT map_template_regions.name, map_template_regions.fname, map_template_regions.lname, map_template_regions.party, map_template_regions.twitter_handle, map_template_regions.prefix, map_template_regions.email, map_template_regions.profile_url, project_fields.field_name, data_field_value.field_value, data_field_value.field_id FROM `data_field_value`,project_fields,map_template_regions WHERE  data_field_value.`pro_id`='".$project_id."' AND map_template_regions.id=data_field_value.city_id AND data_field_value.field_id=project_fields.id_project_field AND project_fields.isGroup=0 AND data_field_value.city_id IN(SELECT DISTINCT(`city_id`) FROM `data_field_value` WHERE `pro_id`='$project_id') ORDER BY `map_template_regions`.`name` ASC";
		$result = $connection->query($query);
		return $result;
	 }
 function selectThreeTableData($data="*",$table1,$table2,$table3,$on,$on3,$condition="",$order_cloumn="",$asc_decs="",$ofset="", $limit="") {
  ...
  function selectsingleClientbyid($id) {
   ...
   function getFvalue($id) {
	$connection=$this->dbConnect();  
	$query = "SELECT * FROM `data_field_value` WHERE `field_id` = $id";
	$result = $connection->query($query);
...
//$nodeDetails=$srObject->findcountrydata($row['id_map_template']);
...
// $json_string = json_encode($maindata, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT);
echo $json_string;
