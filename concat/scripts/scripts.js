...
(function($, _, Chart) {
    Polimapper.maindata = {};
     ...
     Polimapper.baseUrl = "https://visualisation.polimapper.co.uk";
...
(function ($, _, Chart) {
  ...
  Polimapper.loadDatafieldText = function () {
    ...
    var sequence_noFields = _.sortBy(comparableFields, function (t) {
      return t.sequence_no;
    });
    // var comparableFields = Polimapper.data.dataFields;
    for (var i = 0; i < sequence_noFields.length; i++) {
      if(sequence_noFields[i].isGroup == true){
    $("#data_con").append("<option data-class='isparent' value='"+sequence_noFields[i].name+"' data-field-name='"+sequence_noFields[i].name+"'>"+sequence_noFields[i].name + "</option>");
  }else if(sequence_noFields[i].parentId !=="0"){
    $("#data_con").append("<option class='ischild' value='"+sequence_noFields[i].name+"' data-field-name='"+sequence_noFields[i].name+"'>↪ "+sequence_noFields[i].name + "</option>");
  }else {
    $("#data_con").append("<option value='"+sequence_noFields[i].name+"' data-field-name='"+sequence_noFields[i].name+"'>"+sequence_noFields[i].name + "</option>");
      }
  }
    // for (var i = 0; i < comparableFields.length; ++i) {
    //   var comparableField = comparableFields[i];
...
(function ($, _) {
  ...
  Polimapper.setMapDetails = function () {
    ...
    if(Polimapper.data.nodeDescription === ""){
      $('.node-description-wraper').remove();
    } else if (Polimapper.data.nodeDescription === "<br>"){
      $('.node-description-wraper').remove();
    }else{
...
// select_group
    ...
    if (Polimapper.data.project_status.visibility == 'draft') {
      $('.polimapper').remove();
      $('.protected-project').empty().html('<div class="draft-project"><h2>'+Polimapper.data.title + ' is in draft.</h2>');
    } else {
    ...
    for (var k = 0, len = nodes.length; k < len; k++) {
      ...
      for (var pathIndex = 0; pathIndex < paths.length; pathIndex++) {
        ...
        if (path) {
          ...
          if(field.keyColors[0]){
            var numcolor = parseInt(field.keyColors[0].minKeyValue.replace("+", ""));
          }
          if(value === null){
            Polimapper.addClass(path, "keyColourNull");
          }
            else if(value >= numcolor){
