...
if ((isset($_GET['dataSetKey']) && !empty($_GET['dataSetKey'])) && (isset($_GET['client']) && !empty($_GET['client']) && $_GET['client'] != 'false') || !empty($parameter_url) || isset($_GET['group'])) {
    ...
    public function getSingleDataFieldValue($projectId){
		$connection = $this->dbConnect();
		$query = "SELECT * FROM data_field_value WHERE pro_id =$projectId ORDER BY id ASC LIMIT 1";
		$result = $connection->query($query);
		$rowss = $result->fetch_assoc();
		return $rowss;
	}
	...
	public function getFieldIdSequence($projectId,$city_id){
		$connection = $this->dbConnect();
		$query = "SELECT field_id FROM data_field_value WHERE pro_id =$projectId AND city_id=$city_id";
        $result = $connection->query($query);
	...
	public function selectProjecFieldsBySequence($projectId,$sequence){
		$connection = $this->dbConnect();
        $query = "SELECT * FROM `project_fields` WHERE `id_project` =$projectId AND isGroup=0 ORDER BY FIELD(`id_project_field`,$sequence)";
		$result = $connection->query($query);
        return $result;
    }
	 /******end********/
	...
	// if (!empty($row)) {
		...
		$maindata = array();
		    $get_a_datafield=$srObject->getSingleDataFieldValue($row['id_project']);
			$city_id=$get_a_datafield['city_id'];
			$sequenced_id=$srObject->getFieldIdSequence($row['id_project'],$city_id);
			if(!empty($sequenced_id)){
			$datafieldData=$srObject->selectProjecFieldsBySequence($row['id_project'],$sequenced_id);
			}else{
		...
		//echo "<pre>"; print_r($nodeDetails);die();
		$clientlogo = null;
		$clientlogofile = null;
		if (!empty($clientData)) {
		...
		$maindata['hideEmptyValOnNode'] = !empty($row['hide_empty_valu_on_node']) && ($row['hide_empty_valu_on_node'] == 'yes') ? true  : false;
